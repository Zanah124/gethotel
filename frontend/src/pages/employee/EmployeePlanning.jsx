...
import { Calendar, ChevronLeft, ChevronRight, Users, Eye } from 'lucide-react';
import { getPlanning, getWeekStart } from '../../services/employee/planningService';
      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
          <div>
            <h1 className="text-2xl md:text-3xl font-bold text-gray-900 flex items-center gap-2">
              <Calendar className="w-8 h-8 text-[#861D1D]" />
              Planning de l&apos;équipe
            </h1>
            <p className="text-gray-600 mt-1 flex items-center gap-2">
              <Eye size={18} />
              Consultation uniquement — défini par l&apos;administration
            </p>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <button
              ...
              className="px-4 py-2 text-sm font-medium text-[#861D1D] bg-[#861D1D]/10 rounded-lg hover:bg-[#861D1D]/20 transition"
            >
              Cette semaine
            </button>
          </div>
        ...
        {loading ? (
          <div className="flex justify-center items-center py-20">
            <div className="w-12 h-12 border-4 border-[#861D1D]/30 border-t-[#861D1D] rounded-full animate-spin" />
          </div>
        ) : employees.length === 0 ? (
          <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-12 text-center">
            <Users className="w-16 h-16 text-gray-300 mx-auto mb-4" />
            <p className="text-gray-600 text-lg">Aucun planning pour cette semaine.</p>
            <p className="text-gray-500 mt-2">L&apos;administration n&apos;a pas encore renseigné les créneaux.</p>
          </div>
        ) : (
              ...
              <table className="w-full min-w-[800px]">
                <thead>
                  <tr className="bg-[#081F5C] text-white">
                    <th className="text-left py-4 px-4 font-semibold sticky left-0 bg-[#081F5C] z-10 min-w-[180px]">
                      Employé / Poste
                    </th>
                    {JOURS.map((j) => (
                      <th key={j.key} className="text-center py-4 px-2 font-semibold min-w-[120px]">
                        {j.label}
                      </th>
                    ))}
                  </tr>
                ...
                <tbody>
                  {employees.map((emp) => (
                    <tr key={emp.id} className="border-b border-gray-100 hover:bg-gray-50/50">
                      ...
                      {JOURS.map((j) => (
                        <td key={j.key} className="py-2 px-2 text-center text-sm text-gray-700">
                          {formatSlot(emp.slots?.[j.key])}
                        </td>
                      ))}
                    </tr>
                  ))}
